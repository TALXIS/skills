...
| U6 | `ubml walk` sets an element's `reviewStatus`, not only an insight's `status`, and `walk next` shows the elements a claim proposes | The prose in `validate-insights` describing what to show and what to record for the element half of a bundle | needs [ubml#46](https://github.com/TALXIS/ubml/pull/46) and [ubml#47](https://github.com/TALXIS/ubml/pull/47) both merged |
