---
name: testing-unit
description: Scaffolds and runs the fast local test layers — plugin unit tests (pp-plugin-test, FakeXrmEasy 3.x in-memory Dataverse pipeline) and script unit tests (pp-test-script, Jest with a mocked Xrm) — all driven by dotnet test. Use when adding unit tests for Dataverse plugins or web-resource scripts, setting up FakeXrmEasy or Jest tests, or asked to test plugin or form-script logic locally.
---
