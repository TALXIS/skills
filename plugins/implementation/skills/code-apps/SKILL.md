---
name: code-apps
description: Scaffolds a Power Apps code app (Vite + React + TypeScript SPA) with pp-app-code and generates typed offline Dataverse data sources from the local data model with pp-app-code-data. Use when building a custom-UI app on Power Platform, adding a code app to a TALXIS workspace, generating typed models and services for Dataverse tables, or setting up a local npm run dev loop for a code app.
---
