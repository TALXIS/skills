---
name: forms
description: Scaffolds a Dataverse main form locally with txc — form, app registration, then the strict tab → column → section → row → cell → control chain — and edits existing form XML safely. Use when creating a form for a table, adding tabs, sections, fields, or controls to a form, or modifying an existing form's layout in a TALXIS workspace.
---
